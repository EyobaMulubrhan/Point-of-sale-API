...
def get_suppliers(db: Session = Depends(get_db),current_user=Depends(require_manager)):
...
def get_supplier(supplier_id: UUID, db: Session = Depends(get_db),current_user=Depends(require_manager)):
    return supplier_service.get_supplier(db, supplier_id)


@router.post("/", response_model=SupplierResponse, status_code=status.HTTP_201_CREATED)
def create_supplier(data: SupplierCreate, db: Session = Depends(get_db),current_user=Depends(require_manager)):
...
def update_supplier(supplier_id: UUID, data: SupplierUpdate, db: Session = Depends(get_db),current_user=Depends(require_manager)):
    return supplier_service.update_supplier(db, supplier_id, data)


@router.delete("/{supplier_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_supplier(supplier_id: UUID, db: Session = Depends(get_db),current_user=Depends(require_manager)):
